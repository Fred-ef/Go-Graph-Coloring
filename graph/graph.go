...
func Make(nodes []INode, edges []IEdge) (IGraph, error) {
	...
	// check parameter != nil
	if nodes==nil || edges==nil { return nil, errors.New("Error: nil parameter.") }
	for i,n := range nodes{
		if n==nil{ return nil, errors.New("The "+(strconv.Itoa(i))+"-th node is nil.") }
	}
	for i,e := range edges{
		if e == nil { return nil, errors.New("The "+(strconv.Itoa(i))+"-th edge is nil.") }
	}
...
func (this Graph) NeighboursOf(n string) (nodes []INode, e error) {
	if !this.member(n) { return nil, errors.New("There is no node labelled with "+n) }
...
func (this Graph) Paint(colors []string) (e error) {
	if colors==nil { return errors.New("Error: nil parameter.") }
...
func (this Graph) addEdge(n1, n2 string) (e error) {
	...
	} else {e = errors.New("At least one of the given nodes ("+n1+", "+n2+") is not in the graph.")}
